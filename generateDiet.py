...
def checkFitness(productQuantity, nutritionProfile): # check the fitness function of this series of portions

    totalNutrients = totalNutrientValues(productQuantity)
    
    proportions = []

    for nutrient in range(len(totalNutrients)):

        #len(totalNutrients)

        proportions.append(math.fabs(totalNutrients[nutrient]-nutritionProfile[nutrient])/nutritionProfile[nutrient])
...
def guessOptimal(guesses, productData, nutritionProfile, output, maxItems): # guess a combination and compare its fitness to the previous best, first parameter how many guessies it makes
    ...
    for guessCount in range(guesses):
        ...
        guessFitness = checkFitness(guess, nutritionProfile)
